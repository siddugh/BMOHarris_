...
class TermsConditionsViewController: UIViewController {
  ...
  func setupUI() {
    ...
    titleHeaderView.titleLabel.text = "Terms & Conditions"
  ...
  func addTableView() {
    ...
    infoTableView.register(TermsandConditionCell.nib, forCellReuseIdentifier: TermsandConditionCell.identifier)
    infoTableView.separatorStyle = .none
  }
}

extension TermsConditionsViewController: UITableViewDelegate, UITableViewDataSource {
  ...
  func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
    let cell = tableView.dequeueReusableCell(withIdentifier: TermsandConditionCell.identifier, for: indexPath) as! TermsandConditionCell
    cell.termsConditonView?.delegate = self
...
extension TermsConditionsViewController: TermsandCondiViewDelegate {
  func moveNext() {

    let controller = TakePhotoInfoController()
    controller.forSelfie = true
    controller.delegate = self
    let _ = controller.view
    controller.takePhotoInfoView.textLabel1.text = "Take a Selfie and upload it"
    controller.takePhotoInfoView.textLabel2.text = "When you click Open Camera\nyour phone camera app will open"
...
extension TermsConditionsViewController: TakePhotoInfoControllerDelegate {
  func onOpenCamera() {
...
extension TermsConditionsViewController: CameraViewControllerDelegate {
  func selectedImage(image:UIImage?) {
    Helper.dumpAderessIdntity(image: image)    
